...
import "./interfaces/ISbxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ISbxMigrator.sol";

contract SbxMigrator is ReentrancyGuard, ISbxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "MmxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "MmxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "MmxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "MmxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "MmxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "MmxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "MmxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "MmxMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "MmxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "MmxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "MmxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "MmxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(sbxPrice);
        require(mintAmount > 0, "MmxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "MmxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "MmxMigrator: invalid tokenA");
            require(tokenB != address(0), "MmxMigrator: invalid tokenB");
        ...
        ISbxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "MmxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "MmxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "MmxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "MmxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("MmxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "MmxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "MmxMigrator: invalid _action");
