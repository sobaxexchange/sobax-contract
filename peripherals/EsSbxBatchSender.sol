...
contract EsSbxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esSbx;

    constructor(address _esSbx) public {
        admin = msg.sender;
        esSbx = _esSbx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsSbxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esSbx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
