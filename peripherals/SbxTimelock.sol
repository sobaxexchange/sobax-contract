...
import "./interfaces/ISbxTimelock.sol";
...
contract SbxTimelock is ISbxTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "MmxTimelock: forbidden");
        _;
    }

    modifier onlyAdminOrHandler() {
        require(msg.sender == admin || isHandler[msg.sender], "MmxTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "MmxTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "MmxTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "MmxTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "MmxTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "MmxTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "MmxTimelock: invalid _buffer");
        require(_buffer > buffer, "MmxTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }

    function setMaxLeverage(address _vault, uint256 _maxLeverage) external onlyAdmin {
      require(_maxLeverage > MAX_LEVERAGE_VALIDATION, "MmxTimelock: invalid _maxLeverage");
    ...
    function setFundingRate(address _vault, uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external onlyAdmin {
        require(_fundingRateFactor < MAX_FUNDING_RATE_FACTOR, "MmxTimelock: invalid _fundingRateFactor");
        require(_stableFundingRateFactor < MAX_FUNDING_RATE_FACTOR, "MmxTimelock: invalid _stableFundingRateFactor");
    ...
    function setFees(
        ...
        require(_taxBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _taxBasisPoints");
        require(_stableTaxBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _stableTaxBasisPoints");
        require(_mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _mintBurnFeeBasisPoints");
        require(_swapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _swapFeeBasisPoints");
        require(_stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _stableSwapFeeBasisPoints");
        require(_marginFeeBasisPoints < MAX_FEE_BASIS_POINTS, "MmxTimelock: invalid _marginFeeBasisPoints");
        require(_liquidationFeeUsd < 10 * PRICE_PRECISION, "MmxTimelock: invalid _liquidationFeeUsd");
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "MmxTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(vault.whitelistedTokens(_token), "MmxTimelock: token not yet whitelisted");
    ...
    function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode) external onlyAdmin {
        if (excludedTokens[_token]) {
            ...
            require(_inPrivateTransferMode == false, "MmxTimelock: invalid _inPrivateTransferMode");
        }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "MmxTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "MmxTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "MmxTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "MmxTimelock: invalid _action");
