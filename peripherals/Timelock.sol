...
contract Timelock is ITimelock {
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = ISBlpManager(sblpManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(sblpManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(sblpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(sblpManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        // ISBlpManager(sblpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setSBlpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        ISBlpManager(sblpManager).setCooldownDuration(_cooldownDuration);
    }
